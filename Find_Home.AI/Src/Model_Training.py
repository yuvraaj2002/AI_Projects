    ...
    def initialize_model_training(self, X_train, X_test, y_train, y_test):
        try:
            ...
            if model_config_obj.fine_tuning == True:
                ...
                hyper_parms = find_hparms_obj.optimize() # Getting best set of hyper-parameters
                ...
                save_object(file_path=model_config_obj.model_storage_path, obj=Tuned_model)
                return Tuned_model

            else:
                Without_Tuning_model = model.fit(X_train, y_train)  # Train model with base params
                save_object(file_path=model_config_obj.model_storage_path, obj=Without_Tuning_model)
...
def train_model(
    ...
    X_test: np.ndarray,
    y_train: np.ndarray,
    y_test: np.ndarray) -> RegressorMixin:
    """
    Args:
        X_train: np.array,
        X_test: np.array,
    ...
    trained_model = model_train_obj.initialize_model_training(X_train, X_test, y_train, y_test)
