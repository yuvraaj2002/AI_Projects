...
@pipeline(name="train_pipeline", enable_cache=False, settings={"docker": docker_settings})
def train_pipeline():
    """
    Args:
        step_1: DataClass,
    ...
    regressor = train_model(X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)
    R2_score,MAE = evaluation(model=regressor,X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)
